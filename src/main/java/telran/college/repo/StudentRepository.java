...
public interface StudentRepository extends JpaRepository<StudentEntity, Long> {
	@Query("select s from StudentEntity s where s.id not in"
	...
	@Query("select s from StudentEntity s where s.id in"
			+"(select ms.id from MarkEntity m right join m.student ms group by ms.id having count(m.mark) < :count)")
	List<StudentEntity> getStudentsCountMarkLess(long count);
}
